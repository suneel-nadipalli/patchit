...
class Visualizer:
    def __init__(self, class_labels, adv_patch, model, norm_mean=None, norm_std=None):
        """
    ...
    def show_predictions(self, images, labels, patch, n_samples=5, top_k=5):
        ...
        patched_images = self.adv_patch.place_patch(images, patch)
        ...
        for i in range(n_samples):
            ...
            ax = axes[i, 0]
            ax.imshow(img)
            ax.set_title(f"Original: {self.class_labels[label] if isinstance(label, int) else label}")
            ax.axis("off")
            ...
            ax.set_yticklabels([self.class_labels[c] for c in topk_idx_orig])
            ax.invert_yaxis()
            ...
            topk_vals_patch, topk_idx_patch = topk_vals_patch.cpu().numpy(), topk_idx_patch.cpu().numpy()
            ...
            ax.set_yticks(np.arange(top_k))
            ax.set_yticklabels([self.class_labels[c] for c in topk_idx_patch])
            ax.invert_yaxis()
            ax.set_xlabel("Confidence (%)")
